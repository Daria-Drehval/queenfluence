      ...
      updateAddToCartState(formState) {
        ...
        if (addToCartText.length) {
          addToCartText.forEach((element) => {
            if (variant) {
              let variantsArray = document.querySelector('.buttonsTextAddToCart');
              let variantElement = variantsArray.querySelectorAll('p');
              let addToCartButton = document.querySelector('.product__submit__buttons .btn');
              let additionalText = variantsArray.querySelector('.preorder__text');
              let additionalTextDiv = document.createElement("div");
              additionalTextDiv.innerHTML = additionalText.innerHTML;

              variantElement.forEach(elementText => {
                if (variant.id == elementText.getAttribute("value")) {
                  element.innerText = elementText.innerText;

                  if (document.querySelector('.preoder__wrapper')) {
                    document.querySelector('.preoder__wrapper').remove();
                  }
                  
                  if (elementText.getAttribute("data-preorder") == "true") {
                    addToCartButton.after(additionalTextDiv);

                    additionalTextDiv.classList.add('preoder__wrapper')
                  } 
                }
              });
            } else {
